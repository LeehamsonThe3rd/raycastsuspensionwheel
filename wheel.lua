...
function Wheel.new(Component : Part, Chassis : Part)
	...
	self.Offset = self.Component.Weld.C0.Position
...
function Wheel:Update(DeltaTime)
	local SuspensionRayOrigin = (self.Chassis.CFrame*CFrame.new(-self.Offset+(self.WheelRadius*UP))).Position
	local SuspensionRayDirection = -self.Chassis.CFrame.UpVector*(self.WheelRadius+self.Component:GetAttribute("MaxSuspensionLength"))
	...
	if self.SuspensionRay then 
		...
		self.SuspensionForce = UP * (self.StiffnessForce+self.DamperForce)
		...
		self.XForce = self.RotationOnlyWheelDirectionCFrame.RightVector * -self.Velocity.X * self.Component:GetAttribute("WheelFriction")
		self.ZForce = self.RotationOnlyWheelDirectionCFrame.LookVector * self.Velocity.Z * self.Component:GetAttribute("WheelFriction")/3
		...
		self.Component.Weld.C0 = CFrame.new(self.Offset + self.SuspensionRay.Distance*UP - self.WheelRadius*UP*2)
	else
		self.SuspensionLength = self.Component:GetAttribute("MaxSuspensionLength")
		self.Force.Force = ZERO
		
		if not self.Component:GetAttribute("SuspensionVisible") then return end
		self.Component.Weld.C0 = CFrame.new(self.Offset + (self.WheelRadius+self.Component:GetAttribute("MaxSuspensionLength"))*UP - self.WheelRadius*UP*2)
	end
end
